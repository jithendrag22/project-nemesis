...
def run_eod_scanner():
    now = datetime.now()
    print("\n" + "="*80)
    print(f"📈 STRATEGY C: EOD RUNNER SCANNER — {now.strftime('%I:%M:%S %p')}")
    print("="*80)
    ...
    sl = trade_pick['fc_low'] * 0.9985 # 0.15% Buffer
    risk = entry - sl
    ...
    print("\n" + "★"*60)
    print("🚀 OFFICIAL PAPER TRADE INSTRUCTIONS (EOD RUNNER) 🚀")
    print("★"*60)
    ...
    print(f"🛑 STOP LOSS  : ₹{sl:.2f} (Includes 0.15% algo buffer)")
    print(f"🎯 TARGET 1   : ₹{target1:.2f} (Sell {shares//2} shares here)")
    print(f"🏃 EOD RUNNER : Let the remaining {shares - (shares//2)} shares run until 3:15 PM!")
    print("-" * 60)
